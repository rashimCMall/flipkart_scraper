...
async function find() {
  try {
    ...
    while(found && pageNo <= 100) {
      ...
      const response = await axios.get(`https://www.flipkart.com/search?sid=tyy%2C4io&otracker=CLP_Filters&p%5B%5D=facets.type%255B%255D%3DSmartphones&page=${pageNo}`, {
        headers: {
      ...
      $('div[class="_1AtVbE col-12-12"] > div[class="_13oc-S"] > div').each(function (idx, el) {
        ...
        const ram_rom = (a_element.find('.fMghEO > ul > li:first')).text().split("|");
        let ram, rom;

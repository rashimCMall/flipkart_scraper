...
async function find() {
  try {
    ...
    while(found && pageNo <= 100) {
      ...
      const response = await axios.get(`https://www.flipkart.com/air-conditioners/pr?sid=j9e%2Cabm%2Cc54&param=892743&wid=4.productCard.PMU_V2_4&page=${pageNo}`, {
        headers: {
      ...
      $('div[class="_1AtVbE col-12-12"] > div[class="_13oc-S"] > div').each(function (idx, el) {
        ...
        const tonIndex = name_list.indexOf('Ton')
        if(tonIndex != -1)
          capacity = name_list[tonIndex-1];
    ...
    // fs.writeFileSync('./flipkart_airconditioner.csv',csv,"utf-8"); 


    const collectionName = 'flipkartAirconditioner';
